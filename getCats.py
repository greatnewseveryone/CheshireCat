...
def parseIndex(fn= './GUTINDEX.ALL.iso-8859-1.txt'):
    gut_all= []
    book_block= []
    for line in open(fn):
...
def getGutTexts(gut_all, keywords= []):
    books= []
    outf= open('cheshire_ind_filtered.csv', 'w')
    ...
    for bb_ind, book_block in enumerate(gut_all):
        ...
        if text_fnl:
            ...
            if matches:
                open(bb_ind+'.txt', 'w').write(req.text.encode('ascii', 'ignore'))
                outc.writerow({'ind':int(bb_ind), 'title':' '.join(book_block[0].split()[:-1]).encode('ascii', 'ignore')})
